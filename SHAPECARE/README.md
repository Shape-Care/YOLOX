...
Configs live in `SHAPECARE/SHAPECARE_exps/<experiment>/` (e.g. `miap-levicare/`),
base checkpoints in `/shared/shapecare-models/` (and its subfolders, e.g.
`InteractiveSport/`), and training outputs are written to
`YOLOX_outputs/<exp-name>/`.
...
python tools/train.py \
    -f SHAPECARE/SHAPECARE_exps/miap-levicare/yolox_s_124_freeze.py \
...
```bash
python tools/train.py \
    -f SHAPECARE/SHAPECARE_exps/miap-levicare/yolox_s_124_unfreeze.py \
...
python tools/export_onnx.py \
    -f SHAPECARE/SHAPECARE_exps/miap-levicare/yolox_tiny_124_unfreeze.py \
